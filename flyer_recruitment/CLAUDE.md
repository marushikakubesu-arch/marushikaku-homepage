...
- TEL: 059-358-8866
- 営業時間: 月〜金 10:00-16:00（homepage/CLAUDE.mdのGoogleビジネスプロフィール登録情報と一致。フッターのTEL行に「／」区切りで併記、2026-09-17追加）
...
## 最終更新日
2026-09-17（アクセスマップをユーザー提示の実際のGoogleマップに基づいて位置関係を修正。学校の方角の誤り・矢印の向きの誤りを修正。河川の経路もOverpass APIの実ジオメトリに基づいて描き直し。ヘッダー・作業の流れ・選ばれる理由・アクセスの各見出しにhomepageと同じ葉っぱ散らし装飾を計4箇所追加、さらにページ左上・左下の角にも枝状の葉っぱ装飾を追加。選ばれる理由リストの葉っぱアイコンを緑色に変更。フッターに運営会社名「株式会社SOL」と営業時間を追加）
